...
export const getWords = async (complexity: WordsComplexity, limit = 500) => {
    const { data } = await axios.get<string[]>(process.env.NODE_ENV === 'development'
        ? `http://localhost:5000/words?complexity=${complexity}&limit=${limit}`
        : `https://alias-server-sber.vercel.app/words?complexity=${complexity}&limit=${limit}`)
